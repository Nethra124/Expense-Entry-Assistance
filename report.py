...
def insert():
    ...
    return render_template('insert.html', pname = pname, deptname = deptname)

@app.route('/insertexpense')
def insertexpense():
    pname = get_user_name()
    rname = get_report_name()
    return render_template('insertexpense.html', pname = pname, rname = rname)
...
def create():
    ...
    # Create the report
    create_report(report_name, report_date, report_dept, user_name)

    return redirect(url_for('insertexpense'))
